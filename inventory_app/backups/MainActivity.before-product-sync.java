...
public class MainActivity extends Activity {
    ...
    private void productDialog() {
        ...
        new AlertDialog.Builder(this).setTitle("إضافة منتج").setView(l).setPositiveButton("حفظ",(d,w)->{
            try { long r=db.addProduct(name.getText().toString().trim(),cat.getText().toString().trim(),sku.getText().toString().trim(),Integer.parseInt(qty.getText().toString().trim().isEmpty()?"0":qty.getText().toString().trim()),Integer.parseInt(min.getText().toString().trim().isEmpty()?"5":min.getText().toString().trim()),notes.getText().toString().trim()); if(r==-1) Toast.makeText(this,"الكود مستخدم أو البيانات غير صحيحة",Toast.LENGTH_LONG).show(); showProducts(""); } catch(Exception e){Toast.makeText(this,"تحقق من البيانات",Toast.LENGTH_SHORT).show();}
